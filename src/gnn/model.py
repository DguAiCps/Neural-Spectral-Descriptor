...
class SpectralGNN(nn.Module):
    ...
    def __init__(
        ...
        self.sensor_film_enabled = bool((sensor_gate_config or {}).get("film", False))
        if self.sensor_film_enabled:
            self.film_net = nn.Sequential(nn.Linear(3, 32), nn.ReLU(), nn.Linear(32, 2 * context_dim))
            nn.init.zeros_(self.film_net[-1].weight); nn.init.zeros_(self.film_net[-1].bias)
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        if getattr(self, "sensor_film_enabled", False):
            _bc = getattr(data, "beam_count", None)
            if _bc is None:
                _bc = torch.full((context.size(0), 1), 64.0, device=context.device, dtype=context.dtype)
            _bc = _bc.reshape(-1, 1).to(context.dtype)
            _char = torch.cat([torch.log1p(_bc) / 5.0, x_raw.mean(-1, keepdim=True), x_raw.std(-1, keepdim=True)], dim=-1)
            _g, _b = self.film_net(_char).chunk(2, dim=-1)
            context = context * (1.0 + _g) + _b
