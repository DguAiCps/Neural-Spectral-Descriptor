...
def evaluate_cache(
    ...
    if dump_dir is not None:
        ...
        _seq = Path(cache_path).stem
        _out = _os.path.join(dump_dir, _seq + "_dump.npz")
        _kw = dict(raw_descriptors=descriptors.astype("float32"), poses=poses.astype("float32"))
        if embeddings is not None:
            _kw["refined_key"] = embeddings.astype("float32")
            _kw["raw_dim"] = int(raw_dim)
        import numpy as _np; _np.savez_compressed(_out, **_kw)
        print("DUMPED", _out, {k: (v.shape if hasattr(v, "shape") else v) for k, v in _kw.items()})
...
def main() -> None:
    ...
    parser.add_argument("--phase-rerank-mode", default="sketch",
    ...
    parser.add_argument("--dump-dir", default=None,
                        help="if set, save per-seq raw/refined descriptors + poses to this dir")
