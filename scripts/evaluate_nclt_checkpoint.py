...
def _build_nclt_bev_cache(
    ...
    expected_n = int(len(base_cache["scan_ids"]))
    if output_path.exists():
        cached = np.load(output_path)["bev_layouts"].astype(np.float32)
        if cached.shape[0] == expected_n:
            return cached
        print(
            f"[{date}] stale BEV cache at {output_path} "
            f"(cached={cached.shape[0]} vs expected={expected_n}); rebuilding",
            flush=True,
        )
...
def main() -> None:
    ...
    parser.add_argument("--root", default="/workspace/data/nclt")
    ...
    for date in args.dates:
        ...
        bev_path = Path(args.bev_cache_dir) / (
            f"nclt_bev_layout_{date}_s60_{bev_height_encoding}_r1-80_z-3-5_h8_stride{args.scan_stride}.npz"
        )
