#!/usr/bin/env python3
"""Fast MulRan validation for NSD checkpoint/phase-sketch diagnostics."""
...
def _build_mulran_cache(
    ...
    cache_path = cache_dir / f"mulran_operating_{sequence}_layout{layout_sectors}_stride{scan_stride}.npz"
    if cache_path.exists():
        return cache_path

    loader = MulRanLoader(root, sequence, lazy_load=True)
...
def main() -> None:
    ...
    parser.add_argument("--root", default="/workspace/data/mulran")
    parser.add_argument("--sequences", nargs="+", default=["DCC03", "KAIST03", "Riverside03"])
    parser.add_argument("--cache-dir", default="data/preprocessed_mulran_checkpoint_eval")
    parser.add_argument("--bev-cache-dir", default="data/preprocessed_mulran_bev_layout")
    ...
    parser.add_argument(
        ...
        help="Override range-image elevation range for this dataset (default: MulRan Ouster OS1-64).",
    )
    parser.add_argument("--output", default="results/mulran_checkpoint_eval_nointerdiff288_gate00625_phase384_n800.json")
    ...
    for sequence in args.sequences:
        cache_path = _build_mulran_cache(
        ...
        bev_path = Path(args.bev_cache_dir) / (
            f"mulran_bev_layout_{sequence}_s60_{bev_height_encoding}_r1-80_z-3-5_h8.npz"
        )
        bev_layouts = _build_mulran_bev_cache(
