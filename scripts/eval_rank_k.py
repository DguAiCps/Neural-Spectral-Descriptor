#!/usr/bin/env python3
"""Recall@{1,5,10} for the deployed B3 pipeline (review response, 2026-07-21).

Same pipeline as run_b3_rerank.py (pass-1 -> classifier edges -> pass-2 ->
phase-sketch fusion grid), but scores k_values=[1,5,10] for both the coarse
416D ranking and every fusion grid point. Output:
results/_remetrize_twopass/b3_rankk_{seed}.json with per-sequence
{"coarse": {"R@1":..,"R@5":..,"R@10":..}, "grid": {wkey: {...}}}.
"""
...
def rank_k(emb, poses):
    normed = _normalize(emb)
    q = _find_queries(poses, DTH, SKIP)
    ranked = [_topk_cosine(normed, i, max(KS) + 2 * SKIP, SKIP)[: max(KS)] for i, _ in q]
    return _score(poses, q, ranked, KS, DTH)
    ...
    for sensor, seqs in SENSORS.items():
        for seq in seqs:
            ...
            res = _recall_with_phase_sketch_fusion(
                ...
                bev_layouts=bev, poses=poses, k_values=KS, distance_threshold=DTH,
            ...
            grid = {k: {kk: vv for kk, vv in v.items() if kk.startswith("R@")}
                    for k, v in res.items() if not k.startswith("_")}
            report[name] = {"coarse": coarse, "grid": grid}
            print(f"[rankk] {name:<20} coarse R@1/5/10 = "
                  f"{coarse['R@1']:.3f}/{coarse['R@5']:.3f}/{coarse['R@10']:.3f}", flush=True)

    json.dump(report, open(OUT / f"b3_rankk_{seed}.json", "w"), indent=1)
    print("saved", OUT / f"b3_rankk_{seed}.json", flush=True)
