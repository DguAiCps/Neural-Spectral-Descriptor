#!/usr/bin/env python3
"""Fast HeLiPR validation for NSD checkpoint/phase-sketch diagnostics."""
...
def _resolve_helipr_seq_path(root: Path, sequence: str) -> Path:
    """HeLiPR sequences are nested at <root>/<seq>/<seq>/{LiDAR,LiDAR_GT}."""
    nested = root / sequence / sequence
    flat = root / sequence
    if (nested / "LiDAR").exists():
        return nested
    if (flat / "LiDAR").exists():
        return flat
    # Default to nested layout (training script convention); HeLiPRLoader will raise.
    return nested


def _build_helipr_cache(
    root: Path,
    sequence: str,
    config: Dict,
    cache_dir: Path,
    ...
    cache_path = cache_dir / f"helipr_operating_{sequence}_layout{layout_sectors}_stride{scan_stride}.npz"
    if cache_path.exists():
        return cache_path

    seq_path = _resolve_helipr_seq_path(root, sequence)
    loader = HeLiPRLoader(str(seq_path), lazy_load=True)
    ...
    for scan_id in range(0, len(loader), scan_stride):
        if scan_id % 250 == 0:
            print(f"[{sequence}] scan {scan_id}/{len(loader)}, keyframes={len(scan_ids)}", flush=True)
        item = loader[scan_id]
...
def _build_helipr_bev_cache(
    ...
    for i, scan_id in enumerate(base_cache["scan_ids"].astype(np.int64)):
        if i % 250 == 0:
            print(f"[{sequence}] BEV layout {i}/{len(base_cache['scan_ids'])}", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--root", default="/workspace/data/helipr")
    parser.add_argument("--sequences", nargs="+", default=["Town01"])
    parser.add_argument("--cache-dir", default="data/preprocessed_helipr_checkpoint_eval")
    parser.add_argument("--bev-cache-dir", default="data/preprocessed_helipr_bev_layout")
    ...
    parser.add_argument("--output", default="results/helipr_checkpoint_eval_nointerdiff288_gate00625_phase384_n800.json")
    ...
    if args.elevation_range is not None:
        ...
        if args.sensor_key in sensor_ranges:
            ...
            # HeLiPR VLP-16 fallback when sensor_elevation_ranges is missing.
            enc["elevation_range"] = [-15.0, 15.0]
    ...
    for sequence in args.sequences:
        cache_path = _build_helipr_cache(
            Path(args.root), sequence, config, Path(args.cache_dir), args.device,
            layout_sectors=60, scan_stride=args.scan_stride
        )
        base_cache = np.load(cache_path)
        bev_height_encoding = args.bev_height_encoding
        bev_path = Path(args.bev_cache_dir) / (
            f"helipr_bev_layout_{sequence}_s60_{bev_height_encoding}_r1-80_z-3-5_h8.npz"
        )
        bev_layouts = _build_helipr_bev_cache(
            Path(args.root), sequence, base_cache, bev_path,
        ...
        results[sequence] = evaluate_cache(
        ...
        print(sequence, json.dumps(results[sequence], indent=2))
